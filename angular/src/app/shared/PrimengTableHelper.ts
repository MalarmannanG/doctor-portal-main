...
import { ProgressBar } from 'primeng/progressbar/progressbar';
...
export class PrimengTableHelper {
    ...
    tblScrollHeight = '350px';

    ScrollinnerHeightWithOutTiles="250px";
    ...
    getMaxResultCount(paginator: Paginator, event: any): number {
        if (paginator.rows) {
    ...
    getSkipCount(paginator: Paginator, event: any): number {
        if (paginator.first) {
    ...
    shouldResetPaging(event: any): boolean {
