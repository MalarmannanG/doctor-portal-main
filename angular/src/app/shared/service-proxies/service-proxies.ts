...
export class AppointmentController {
     ...
     * @return Success
     */
    getAll(patientName: string | undefined, isProcedure: boolean | undefined, doctorName: string | undefined, clinicName: string | undefined, fromDate: string | undefined, toDate: string | undefined, todayPatients: boolean | undefined, skip: number | undefined, take: number | undefined, order_by: string | undefined): Observable<void> {
        ...
        if (clinicName === null)
            throw new Error("The parameter 'clinicName' cannot be null.");
        else if (clinicName !== undefined)
            url_ += "clinicName=" + encodeURIComponent("" + clinicName) + "&";
        if (fromDate === null)
...
export class TemplateMasterController {
     ...
     * @return Success
     */
    getAll(name: string | undefined, sorting: string | undefined, skipCount: number | undefined, maxResultCount: number | undefined): Observable<TemplateMasterModelPagedResultDto> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<TemplateMasterModelPagedResultDto>;
                }
            } else
                return _observableThrow(response_) as any as Observable<TemplateMasterModelPagedResultDto>;
        }));
    }

    protected processGetAll(response: HttpResponseBase): Observable<TemplateMasterModelPagedResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = TemplateMasterModelPagedResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<TemplateMasterModelPagedResultDto>(null as any);
    }
...
export class TemplateMasterModelPagedResultDto implements ITemplateMasterModelPagedResultDto {
    items!: TemplateMasterModel[] | undefined;
    totalCount!: number;

    constructor(data?: ITemplateMasterModelPagedResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            if (Array.isArray(_data["items"])) {
                this.items = [] as any;
                for (let item of _data["items"])
                    this.items!.push(TemplateMasterModel.fromJS(item));
            }
            this.totalCount = _data["totalCount"];
        }
    }

    static fromJS(data: any): TemplateMasterModelPagedResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new TemplateMasterModelPagedResultDto();
        result.init(data);
        return result;
    }
...
export interface ITemplateMasterModelPagedResultDto {
    items: TemplateMasterModel[] | undefined;
    totalCount: number;
}
